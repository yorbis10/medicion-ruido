...
class FirebaseAuthManager extends AuthManager
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            'pf6i6bvm' /* Ha pasado demasiado tiempo des... */,
          ))),
        );
      }
    }
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(FFLocalizations.of(context).getText(
            'edsz3fq0' /* Ha pasado demasiado tiempo des... */,
          ))),
        );
      }
    }
  ...
  Future resetPassword({
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text(FFLocalizations.of(context).getText(
        'tkiv6fyr' /* Se envió  correo electrónico d... */,
      ))),
    );
  }
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : ContaminacionAcusticaFirebaseUser.fromUserCredential(
              userCredential);
    } on FirebaseAuthException catch (e) {
      final errorMsg = switch (e.code) {
        'email-already-in-use' => FFLocalizations.of(context).getText(
            '2fh23tou' /* Correo electrónico ya en uso p... */,
          ),
        'INVALID_LOGIN_CREDENTIALS' => FFLocalizations.of(context).getText(
            'ohsoj2e3' /* la credencial de autenticación... */,
          ),
        _ => 'Error: ${e.message!}',
      };
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
